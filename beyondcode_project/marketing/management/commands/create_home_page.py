...
class Command(BaseCommand):
    help = 'Create or update the home page with the "One Platform for Compliant AI Collections" content'

    def handle(self, *args, **options):
        # Check if home page exists
        try:
            home_page = Page.objects.get(slug='home')
            self.stdout.write(f'Home page already exists: {home_page.title}')
            return
        except Page.DoesNotExist:
            self.stdout.write('Creating home page...')

        # Create the home page with the content
        blocks_json = {
            'blocks': [
                {
                    'type': 'feature_grid',
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Successfully created home page: {page.title}')
        )