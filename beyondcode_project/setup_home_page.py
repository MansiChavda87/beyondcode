...
def check_and_create_home_page():
    print("Checking for existing home page...")

    try:
        home_page = Page.objects.get(slug='home')
        print(f"Home page already exists: {home_page.title}")
        print(f"Status: {home_page.status}")
        print(f"Has blocks: {bool(home_page.blocks_json)}")
        return
    except Page.DoesNotExist:
        print("Home page does not exist. Creating...")

    # Create the home page with the "One Platform for Compliant AI Collections" content
    blocks_json = {
        'blocks': [
            {
                'type': 'feature_grid',
    ...
    print(f"Successfully created home page: {page.title}")
    print(f"Page ID: {page.id}")
    print(f"Slug: {page.slug}")
    print(f"Status: {page.status}")

if __name__ == '__main__':
    check_and_create_home_page()